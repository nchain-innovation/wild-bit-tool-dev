...
def load_key_from_file(filename:str, toml:bool, key_type:str) -> tuple[str, str] :
    ...
    try:
        if toml:
            ...
            key = key_info['key_info']['private_key'], key_info['key_info']['bitcoin_address']

        # else deal with pem format
        else:
            key_str = read_file(filename, False)
            key_as_bytes = key_str.encode()
            privkey = PrivateKey.from_pem(key_as_bytes, network=key_type)
            key = key_to_wif(privkey), privkey.address
    ...
    for file in os.listdir(path=path):
        if file.endswith(".key"):
            ...
            if 'key_info' in data_dict:
                ...
                # create a "PrivateKey" object and check the network matches
                key = wif_to_key(data_dict["key_info"]["private_key"])
                if key.network == "main":
                    network_type = "mainnet"
                elif key.network == "test":
                    network_type = "testnet"
                else:
                    print(f"Error: Invalid network type: {key.network}")
                    exit(1)
