...
class TestPkeyformatCommand(unittest.TestCase):
    ...
    def test_wif_to_int(self, mock_exit, mock_stdout):
        ...
        self.assertEqual(addr_cmd.key_type, 'BSV_Testnet')
    ...
    @patch('pathlib.PosixPath.is_file', return_value=True)
    @patch('sys.stdout', new_callable=StringIO)
    def test_pem_to_hex(self, mock_stdout, mock_is_file, mock_open_file, mock_exit):
        ...
        self.assertEqual(addr_cmd.key_type, 'BSV_Mainnet')
        assert addr_cmd.private_key is None
        # Run the command and handle SystemExit
        with self.assertRaises(SystemExit):
            addr_cmd.run()
