...
from balance_command import BalanceCommand, BunchOfBalances
...
class TestBalanceCommand(unittest.TestCase):
    ...
    def test_bitcoin_balance_creation(self):
        ...
        self.assertEqual('toml', bb.inform)
        self.assertEqual('test', bb.key_type)


    # test get_balance with address parameter
    def test_get_balance(self):
        balance = self.bb.get_balance()
        assert balance == 0
    ...
    # test print balance with a zero balance 
    def test_print_balance(self):
        expected_output = '\n------------------------------------------------------------------------------------\nPubKey: \tmy_address\nBalance: \t0\n------------------------------------------------------------------------------------\n\n'
        with patch('sys.stdout', new=StringIO()) as fake_out:
            self.bb.print_balance()
            self.assertEqual(fake_out.getvalue(), expected_output)
    ...
    def test_load_key_from_file(self, mock_open_file, mock_is_file):
        ...
        assert self.bb.address == "msJjhsacnGxvzFMHBXoqdr63DhVRXdPQjS"
    ...
    def test_BunchOfBalances(self):
        pass

    def test_check_balances(self):
        pass

    # also add test for list all balances for mainnet/testnet
